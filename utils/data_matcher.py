...
def cruzar_emissao_com_chegada(df_emissao, df_chegada):
    ...
    # Busca coluna de Nota Fiscal ignorando case
    nota_col_chegada = next(
        (col for col in df_chegada.columns if col.strip().lower() == "nota fiscal"),
        None
    )

    if not nota_col_chegada:
        raise ValueError("❌ Coluna 'Nota Fiscal' não encontrada na planilha de chegada!")
